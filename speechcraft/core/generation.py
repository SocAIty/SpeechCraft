...
from speechcraft.settings import MODELS_DIR, USE_SMALL_MODELS, GLOBAL_ENABLE_MPS, OFFLOAD_CPU
import os
import logging
...
def generate_text_semantic(
    ...
    if history_prompt is not None:
        if not isinstance(history_prompt, VoiceEmbedding):
            history_prompt = VoiceEmbedding.load(history_prompt)
    ...
    with _inference_mode():
        x = x.to(device)
        n_tot_steps = 768
        # if no progress_update_func, use tqdm
        # custom tqdm updates since we don't know when eos will occur
        progress_update_func = _get_progress_update_func(progress_update_func)
        ...
        for n in range(n_tot_steps):
            ...
            if allow_early_stop and (
                ...
                # eos found, so break
                progress_update_func(1.0)
                break
            x = torch.cat((x, item_next[None]), dim=1)
            tot_generated_duration_s += 1 / SEMANTIC_RATE_HZ
            # hard cut if audio becomes too long (>max_gen_duration)
            ...
            if n > n_tot_steps:
                ...
                progress_update_func(round(n / n_tot_steps, 2))
...
def _get_progress_update_func(progress_update_func: callable = None):
    # replace progress_update_func with tqdm if not provided to show progress
    if progress_update_func is not None:
        return progress_update_func

    # return a tqdm object to display progress.
...
def generate_coarse(
        ...
        use_kv_caching=False,
        progress_update_func=None  # function will be called with progress_update_func(x) x in [0, 1]
):
    ...
    with _inference_mode():
        ...
        for i in range(n_window_steps):
            # update progress bar
            progress_update_func(round(i / n_window_steps, 2))
...
def generate_fine(
    ...
    with _inference_mode():
        ...
        for n in range(n_loops):
            # update progress bar
            progress_update_func(round(n / n_loops, 2))
